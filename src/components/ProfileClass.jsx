import { Component } from "react";

export default class Profile extends Component {
    constructor(props) {
        super(props)
    }

    render() {
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
                <h1>About Me</h1>
                <img style={{ borderRadius: '50%', height: '15rem', width: '15rem'}} src="https://avatars.githubusercontent.com/u/129498812?v=4" alt="" />
                <div style={{display: 'flex'}}>
                    <p>React.js |</p>
                    <p>Next.js |</p>
                    <p>Linkedin</p>
                    <p>Javascript |</p>
                    <p>Front End Developer</p>
                </div>
                <div style={{display: 'flex'}}>
                    <img style={{height: '5rem', width: '5rem', margin: '0.5rem'}} src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRFV_OwzztZQ61npDDy2m94TYnmn5bXl0PkXQ&s" alt="" />
                    <img style={{height: '5rem', width: '5rem', margin: '0.5rem'}} src="data:image/png;base64," alt="" />
                    <img style={{height: '5rem', width: '5rem', margin: '0.5rem'}} src="https://i.pinimg.com/736x/b5/1b/78/b51b78ecc9e5711274931774e433b5e6.jpg" alt="" />
                </div>
            </div>
        )
    }
}